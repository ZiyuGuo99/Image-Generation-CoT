...
class Showo(ModelMixin, ConfigMixin):
    ...
    def t2i_generate_dpo(
        ...
        for step in range(timesteps):
            ...
            input_ids_minus_lm_vocab_size = torch.where(masking, mask_token_id, sampled_ids)
            if timesteps == 1:
                total_input_ids.append(input_ids.unsqueeze(1))
            if step != timesteps - 1:
                total_masking.append(torch.bitwise_xor(unknown_map, masking).unsqueeze(1))
                total_input_ids.append(input_ids.unsqueeze(1).clone())
            else:
                total_masking.append(torch.bitwise_xor(unknown_map, torch.zeros_like(unknown_map)).unsqueeze(1))
            total_selected_probs.append(selected_probs.unsqueeze(1))
            total_sampled_ids.append(sampled_ids.unsqueeze(1))

        return sampled_ids, torch.cat(total_sampled_ids, dim=1), torch.cat(total_input_ids, dim=1)
